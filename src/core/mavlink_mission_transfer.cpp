...
{
    _work_queue.emplace_back(std::unique_ptr<WorkItem>(new UploadWorkItem(
...
{
    _work_queue.emplace_back(std::unique_ptr<WorkItem>(new DownloadWorkItem(
        _config, _sender, _message_handler, _timeout_handler, type, callback)));
}

void MAVLinkMissionTransfer::do_work()
{
    LockedQueue<std::unique_ptr<WorkItem>>::Guard work_queue_guard(_work_queue);
